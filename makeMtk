...
while ($#ARGV != -1)
{
  ...
  else
  {
    ...
    chomp ($productMakefile = `/bin/ls device/*/*/${project}.mk 2>/dev/null`);
    $ENV{MTK_HYBRID_PRODUCTFILE} = "$productMakefile";
    if ($productMakefile ne "") { # Hybrid build system
      open (CURFILE, $productMakefile) or die "Cannot open $productMakefile: $!\n";
      while (<CURFILE>) {
        $targetProduct = $1 if (/^\s*PRODUCT_NAME\s*:=\s*(\S+)/);
    ...
    if ($baseProject ne "") {
      	chomp(my $curDir = `pwd`);
        print "Create symbol link ${targetProduct} based on ${baseProject}...\n";
        system ("ln -s ${curDir}/mediatek/config/${baseProject} mediatek/config/${targetProduct}") if ((! -e "mediatek/config/${targetProduct}") && (-e "mediatek/config/${baseProject}"));
        system ("ln -s ${curDir}/mediatek/custom/${baseProject} mediatek/custom/${targetProduct}") if ((! -e "mediatek/custom/${targetProduct}") && (-e "mediatek/custom/${baseProject}"));
        system ("ln -s ${curDir}/bootable/bootloader/lk/project/${baseProject}.mk bootable/bootloader/lk/project/${targetProduct}.mk") if ((! -e "bootable/bootloader/lk/project/${targetProduct}.mk") && (-e "bootable/bootloader/lk/project/${baseProject}.mk"));
    ...
    die "Can NOT find project Makefile "
        . "mediatek/config/${project}/ProjectConfig.mk!\nPlease create it or specify BASE_PROJECT in your product makefile!\n" 
...
foreach $prj ("mediatek/config/${project}/ProjectConfig.mk", "mediatek/config/common/ProjectConfig.mk", $productMakefile)
{
  next if ($prj eq "");
