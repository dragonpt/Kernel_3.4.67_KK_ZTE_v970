...
#else
//zhuoshineng add for no ntc battry
       return 30;
//add end
#endif
    if(dwVolt > dwVCriBat)
...
//zhaoshaopeng from mtk
    ...
    (BMT_status.charger_protect_status == BATTERY_OVER_TEMP))
 {
    ...
    BMT_status.charger_protect_status = 0;
        if (Enable_BATDRV_LOG == 1) {
        printk(  "[BATTERY] temperture in range... start charging again!!\n\r");
    }
 }
 else
 	{printk(  "[BATTERY] temperture in range... how many times does this is called!!\n\r");}
    return PMU_STATUS_OK;
}
