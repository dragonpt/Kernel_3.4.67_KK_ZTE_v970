    ...
    int **gpuloading)
{
    ...
    if (gpufreq)
    {
        gpufreqs[0] = 0; // No info
        *gpufreq = gpufreqs;
    }

    if (gpuloading)
    {
        gpuloadings[0] = 0; // No info
    ...
    unsigned int mask)
{
    ...
    {
        //set_taklking_flag(true); // make mtk_ts_cpu.c aware of call scenario // Not available in 77
    ...
    unsigned int mask)
{
    if ((mask & MTK_THERMAL_SCEN_CALL)) // only one scen is handled now...
    {
        //set_taklking_flag(false); // make mtk_ts_cpu.c aware of call scenario // Not available in 77
        _thermal_scen &= ~((unsigned int) MTK_THERMAL_SCEN_CALL);
    }
    return _thermal_scen;
}
