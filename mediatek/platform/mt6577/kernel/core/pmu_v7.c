...
void armv7_pmnc_dump_regs(void)
{
        ...
        printk(KERN_ALERT "PMNC registers dump(CPU ID = %d):\n", raw_smp_processor_id());

        asm volatile("mrc p15, 0, %0, c9, c12, 0" : "=r" (val));
        printk(KERN_ALERT "PMNC  =0x%08x\n", val);

        asm volatile("mrc p15, 0, %0, c9, c12, 1" : "=r" (val));
        printk(KERN_ALERT "CNTENS=0x%08x\n", val);

        asm volatile("mrc p15, 0, %0, c9, c14, 1" : "=r" (val));
        printk(KERN_ALERT "INTENS=0x%08x\n", val);

        asm volatile("mrc p15, 0, %0, c9, c12, 3" : "=r" (val));
        printk(KERN_ALERT "FLAGS =0x%08x\n", val);

        asm volatile("mrc p15, 0, %0, c9, c12, 5" : "=r" (val));
        printk(KERN_ALERT "SELECT=0x%08x\n", val);

        asm volatile("mrc p15, 0, %0, c9, c13, 0" : "=r" (val));
        printk(KERN_ALERT "CCNT  =0x%08x\n", val);

        for (cnt = ARMV7_COUNTER0; cnt < ARMV7_COUNTER_LAST; cnt++) {
                armv7_pmnc_select_counter(cnt);
                asm volatile("mrc p15, 0, %0, c9, c13, 2" : "=r" (val));
                printk(KERN_ALERT "CNT[%d] count =0x%08x\n",
                        cnt-ARMV7_EVENT_CNT_TO_CNTx, val);
                asm volatile("mrc p15, 0, %0, c9, c13, 1" : "=r" (val));
                printk(KERN_ALERT "CNT[%d] evtsel=0x%08x\n",
                        cnt-ARMV7_EVENT_CNT_TO_CNTx, val);
        }
}
#endif
...
static void armv7pmu_write_all_counter(void)
{
	int idx, cpu = raw_smp_processor_id();
	struct  pmu_data *p_data = (struct  pmu_data *) &armv7pmu.perf_data;
	
	for (idx = 0; idx < NUMBER_OF_EVENT + 1; idx++) {
        armv7pmu_write_counter(idx, p_data->cnt_val[cpu][idx]);
   	} 
...
void pmu_regs_save(void)
{
    armv7pmu_read_all_counter(NULL);
}

void pmu_regs_restore(void)
{
    armv7pmu_enable(NULL);
    armv7pmu_write_all_counter();
}  
