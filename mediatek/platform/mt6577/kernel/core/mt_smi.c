...
static int MT_SMI_probe(struct platform_device *pdev)
{
    ...
    smimon_device = device_create(pSMI_CLASS , NULL, g_SMIdevno , NULL, "mt-smi");
...
static int MT_SMI_remove(struct platform_device *pdev)
{
...
static int MT_SMI_suspend(struct platform_device *pdev, pm_message_t mesg)
{
    return 0;
}

static int MT_SMI_resume(struct platform_device *pdev)
{
    return 0;
}
...
static struct platform_driver g_smi_platform_driver = 
{
    .probe		= MT_SMI_probe,
    .remove     = MT_SMI_remove,
    .suspend	= MT_SMI_suspend,
    .resume	    = MT_SMI_resume,
    .driver		= 
    {
        .name	= "mt-smi",  /* FIXME. Sync to MHAL and init.rc */  /*Is this the name of device file under /dev/ ? */
        .owner	= THIS_MODULE,
    }
};
...
//    printk("Insert MT6575 bus monitor module\n");
    ...
    if(platform_driver_register(&g_smi_platform_driver))
    {
        printk("failed to register SMI driver\n");
        return -ENODEV;
    }
