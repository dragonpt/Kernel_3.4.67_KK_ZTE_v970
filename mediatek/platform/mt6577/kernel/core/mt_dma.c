...
void gdma2_check_mpu_violation(u32 addr, int wr_vio)
{
    printk(KERN_CRIT "GDMA2 checks EMI MPU violation.\n");
    printk(KERN_CRIT "addr = 0x%x, %s violation.\n", addr, wr_vio? "Write": "Read");
    printk(KERN_CRIT "DMA SRC = 0x%x.\n", readl(DMA_SRC(DMA_BASE_CH(1))));
    printk(KERN_CRIT "DMA DST = 0x%x.\n", readl(DMA_DST(DMA_BASE_CH(1))));
    printk(KERN_CRIT "DMA COUNT = 0x%x.\n", readl(DMA_LEN1(DMA_BASE_CH(1))));
    printk(KERN_CRIT "DMA CON = 0x%x.\n", readl(DMA_CON(DMA_BASE_CH(1))));
}
...
static int __init mt_init_dma(void)
{
    ...
    emi_mpu_notifier_register(MST_ID_DMA, gdma1_check_mpu_violation);
    emi_mpu_notifier_register(MST_ID_DMA, gdma2_check_mpu_violation);
