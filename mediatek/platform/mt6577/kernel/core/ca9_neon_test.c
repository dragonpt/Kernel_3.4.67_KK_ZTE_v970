...
extern int ca9_neon(int cpu_id);
extern int ca9_neon_indicative(int cpu_id);
extern int max_power_neon_loop(int cpu_id);

static unsigned int g_iNeonPassFail_CPU0;
static unsigned int g_iNeonPassFail_CPU1;
static unsigned int g_iNeonLoopCount;
...
int ca9_neon_test(int cpu_id)
{
    ...
    {    
        ...
        {            
#if 1  
            iV = ca9_neon_indicative(cpu_id); 
               
            if(iV == 1)
            {    
#endif                             
                iV = max_power_neon_loop(cpu_id);
...
static struct device_driver ca9_neon_loop_count_drv =
{
    .name = "ca9_neon_loop_count",
    .bus = &platform_bus_type,
    .owner = THIS_MODULE,
};
...
{
    if(g_iNeonPassFail_CPU0 == -1)
        return snprintf(buf, PAGE_SIZE, "CPU0 CA9_NEON - CPU0 is powered off\n");
    else
        return snprintf(buf, PAGE_SIZE, "CPU0 CA9_NEON - %s(loop_count = %d)\n", g_iNeonPassFail_CPU0 != g_iNeonLoopCount ? "FAIL" : "PASS", g_iNeonPassFail_CPU0);
}

static ssize_t cpu1_slt_ca9_neon_show(struct device_driver *driver, char *buf)
{
    if(g_iNeonPassFail_CPU1 == -1)
        return snprintf(buf, PAGE_SIZE, "CPU1 CA9_NEON - CPU1 is powered off\n");
    else
        return snprintf(buf, PAGE_SIZE, "CPU1 CA9_NEON - %s(loop_count = %d)\n", g_iNeonPassFail_CPU1 != g_iNeonLoopCount ? "FAIL" : "PASS", g_iNeonPassFail_CPU1);   
}

static ssize_t ca9_neon_loop_count_show(struct device_driver *driver, char *buf)
{
    return snprintf(buf, PAGE_SIZE, "CA9 NEON Loop Count = %d\n", g_iNeonLoopCount);    
}
...
static ssize_t cpu0_slt_ca9_neon_store(struct device_driver *driver, const char *buf, size_t count)
{
    ...
    for (i = 0, g_iNeonPassFail_CPU0 = 0; i < g_iNeonLoopCount; i++) {
        ret = ca9_neon_test(0);  // 1: PASS, 0:Fail, -1: target CPU power off
        if(ret != -1)
        {      
            g_iNeonPassFail_CPU0 += ret;
        }  
        else
        {
            g_iNeonPassFail_CPU0 = -1;
    ...
    if (g_iNeonPassFail_CPU0 == g_iNeonLoopCount) {
...
static ssize_t cpu1_slt_ca9_neon_store(struct device_driver *driver, const char *buf, size_t count)
{
    ...
    for (i = 0, g_iNeonPassFail_CPU1 = 0; i < g_iNeonLoopCount; i++) {
        ret = ca9_neon_test(1);  // 1: PASS, 0:Fail, -1: target CPU power off
        if(ret != -1)
        {      
            g_iNeonPassFail_CPU1 += ret;
        }  
        else
        {
            g_iNeonPassFail_CPU1 = -1;
    ...
    if (g_iNeonPassFail_CPU1 == g_iNeonLoopCount) {
...
static ssize_t ca9_neon_loop_count_store(struct device_driver *driver, const char *buf, size_t count)
{  
    int result;
     
    if ((result = sscanf(buf, "%d", &g_iNeonLoopCount)) == 1)
    {
        printk("set CA9 Neon test loop count = %d successfully\n", g_iNeonLoopCount);
    }
    else
    {
        printk("bad argument!!\n");
...
DRIVER_ATTR(ca9_neon_loop_count, 0644, ca9_neon_loop_count_show, ca9_neon_loop_count_store);
...
int __init ca9_neon_loop_count_init(void)
{
    int ret;

    ret = driver_register(&ca9_neon_loop_count_drv);
    if (ret) {
        printk("fail to create CA9 NEON loop count driver\n");
    }
    else
    {
        printk("success to create CA9 NEON loop count driver\n");
    }

    ret = driver_create_file(&ca9_neon_loop_count_drv, &driver_attr_ca9_neon_loop_count);
    if (ret) {
        printk("fail to create CA9 NEON loop count sysfs files (CPU1)\n");
    }
    else
    {
        printk("success to create CA9 NEON loop count sysfs files (CPU1)\n");
    }    

    g_iNeonLoopCount = SLT_LOOP_CNT;
...
arch_initcall(ca9_neon_loop_count_init);