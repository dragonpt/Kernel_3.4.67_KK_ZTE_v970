...
static void mt_irq_ack(struct irq_data *data)
{
    ...
    __raw_writel(irq, NS_GIC_CPU_BASE + GIC_CPU_EOI);
    dsb();
}
...
#define FIQ_DEBUG_HW_REBOOT 1
#endif
#if defined(FIQ_DEBUG_HW_REBOOT)
static u32 __ctrl, __count, __load;
void __check_wdt(void *regs, void *svc_sp)
{
    if (__count == 0) {
        ...
        printk(KERN_ERR "Local WDT timeouts but spurious interrupt!?\n");
        printk(KERN_ERR "ctrl = 0x%x, count = 0x%x, load = 0x%x\n", __ctrl, __count, __load);
...
#if defined(FIQ_DEBUG_HW_REBOOT)
    __count = *(volatile u32 *)(0xF000A600 + 0x24);
#endif
