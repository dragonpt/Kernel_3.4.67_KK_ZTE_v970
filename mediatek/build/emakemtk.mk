...
REMAKECMD    :=  make -f$(MTK_ROOT_BUILD)/emakemtk.mk CMD_ARGU=$(CMD_ARGU) $(CMD_ARGU2) $(MAKE_DEBUG)
REMAKECMD_E  :=  emake -f$(MTK_ROOT_BUILD)/emakemtk.mk CMD_ARGU=$(CMD_ARGU) $(CMD_ARGU2) $(MAKE_DEBUG)
...
ALL_CUSTIMAGE_CLEAN_FILES := \
        $(LOGDIR)/$(PROJECT)/cust.img \
        $(LOGDIR)/$(PROJECT)/cust \
...
clean-custimage:
	...
	$(hide) rm -rf $(ALL_CUSTIMAGE_CLEAN_FILES)
...
ifeq ($(MTK_DEPENDENCY_AUTO_CHECK), true)
	...
	$(hide) echo -e \\t\\t\\t\\b\\b\\b\\bLOG: $(S_LOG)$@.log
	$(hide) rm -f $(LOG)$@.log $(LOG)$@.log_err
	$(hide) ./emakeMtk $(FULL_PROJECT) mm build/target/board/ snod $(DEAL_STDOUT_UPDATE_MD) && \
         $(SHOWRSLT) $${PIPESTATUS[0]} $(LOG)$@.log || \
         $(SHOWRSLT) $${PIPESTATUS[0]} $(LOG)$@.log
...
custgen: $(PRJ_MF)
	...
	$(hide) emake $(MAKEJOBS) $(MAKE_DEBUG) MTK_CUSTGEN_ERROR=no -f $(MTK_ROOT_BUILD)/custgen.mk $(DEAL_STDOUT_CUSTGEN) && \
