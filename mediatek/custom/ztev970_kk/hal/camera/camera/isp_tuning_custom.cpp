...
//
char * mod_cam = "/sys/module/kd_sensorlist/parameters/sensorname_main";
static MINT32 m_i4FluorescentIndex;
static MINT32 DAYLIGHT_F_IDX = 80;
/*******************************************************************************
*
*   rCamInfo
...
//                                THA TH1 THB              THC TH2  THD
    ...
    char senname[32]={0};
	int is_ov8825_truly=0;
    int fd = open(mod_cam, O_RDONLY);

    int res=read(fd, senname, sizeof(senname));
  //  LOGD("wmhai test res %d senname = %s",res,senname);
    if(strstr(senname,"ov8825-truly-MM"))
	{
		is_ov8825_truly=1;
	}
	else if(strstr(senname,"ov8825-shunyu-MM"))
	{
	       THA = 3518;
	       TH1 = 3864;
	       THB = 4067;
...
//                   THH2  TH2  THL2                   THH1  TH1  THL1
  ...
  //  LOGD("wmhai test res %d senname = %s",res,senname);
    ...
    close(fd);
    //LOGD("wmhai THL2 res %d THH2 = %d DAYLIGHT_F_IDX = %d",THL2,THH2,DAYLIGHT_F_IDX);
