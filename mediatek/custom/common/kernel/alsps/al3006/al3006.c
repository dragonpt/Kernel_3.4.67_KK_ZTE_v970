...
static int al3006_power_down(struct i2c_client *client, bool enable);
static int al3006_powerdown_flag = 0;
#endif
...
int al3006_init_device(struct i2c_client *client)
{
    ...
    if(hwmsen_write_byte(client,APS_ALS_CTL,0xA5))//als 64 levels & low lux threshold
    {
      return -EFAULT;
    }
...
static int al3006_power_down(struct i2c_client *client, bool enable)
{
    ...
    if (al3006_powerdown_flag != enable)
    {
	...
	regdata &= 0b11110011; //first set 00 to clear bit

	if (enable)
	{
	    APS_LOG("al3006_power_down on!\n");
	    regdata |= 0b00001000; //enable both
	}
	else
	{
    // 00 => Power up mode
	    APS_LOG("al3006_power_down off!\n");
	}

	if(hwmsen_write_byte(client,APS_CONFIGUATION,regdata))
	{
		APS_LOG("al3006_power_down failed!\n");
		return -1;
	}
...
void al3006_eint_func(void)
{
	...
	if(!obj)
	{
	 ...
	 wake_lock_timeout(&ps_lock, 3*HZ);
...
#ifdef ALSPS_POWER_DOWN_ENABLE
	if (obj->ps_enable != 1 && obj->als_enable != 1)
	{
	    if (al3006_powerdown_flag == 0)
		mt_eint_unmask(CUST_EINT_ALS_NUM);  
	    
	    APS_LOG("al3006_eint_work All sensor OFF\n");
	    return;
	}
#endif
...
#ifdef ALSPS_POWER_DOWN_ENABLE				
//allow interrupt to be complete		
		mt_eint_mask(CUST_EINT_ALS_NUM);

		msleep(50);
		al3006_power_down(client,true);
#endif
...
#ifdef ALSPS_POWER_DOWN_ENABLE
	al3006_power_down(client,false);
#endif
	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			{				
				...
				{
#ifdef ALSPS_POWER_DOWN_ENABLE
					if (al3006_powerdown_flag == 1)
					{
					    al3006_power_down(obj->client,false);
					}
#endif
					if(err = al3006_enable_ps(obj->client, true))
					{
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			{
#ifdef ALSPS_POWER_DOWN_ENABLE
				while(atomic_read(&obj->als_suspend))
				{
				    msleep(50);
				}
#endif
