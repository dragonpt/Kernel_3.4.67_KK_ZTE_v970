...
static int bma222E_client_resume(struct i2c_client *client, int reset_cali)
{

	struct bma222E_i2c_data *obj = i2c_get_clientdata(client);
	int res = 0;
	printk("bma222E_client_resume \n");
	...
	switch (command)
	{
		case SENSOR_DELAY:
			...
			else
			{
				...
				err = BMA222E_SetBWRate(priv->client, BMA222E_BW_25HZ);
...
static int bma222E_resume(struct i2c_client *client)
{
	...
	if(err = bma222E_client_resume(client, 0))
	{
...
static void bma222E_late_resume(struct early_suspend *h)
{
	...
	BMA222E_power(obj->hw, 1);
	if(err = bma222E_client_resume(obj->client, 0))
	{
		GSE_ERR("initialize client fail!!\n");
		return;        
	}
...
{
	GSE_FUN();
     i2c_register_board_info(0, &i2c_bma222E, 1);
