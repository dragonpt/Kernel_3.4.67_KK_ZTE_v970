...
static int bma250_client_resume(struct i2c_client *client, int reset_cali)
{
	struct bma250_i2c_data *obj = i2c_get_clientdata(client);
	int res = 0;
	printk("bma250_client_resume \n");
	...
	switch (command)
	{
		case SENSOR_DELAY:
			...
			else
			{
				...
				err = BMA250_SetBWRate(priv->client, BMA250_BW_25HZ);
...
static int bma250_resume(struct i2c_client *client)
{
	...
	if(err = bma250_client_resume(client, 0))
	{
...
static void bma250_late_resume(struct early_suspend *h)
{
	...
	BMA250_power(obj->hw, 1);
	if(err = bma250_client_resume(obj->client, 0))
	{
		GSE_ERR("initialize client fail!!\n");
		return;        
	}
