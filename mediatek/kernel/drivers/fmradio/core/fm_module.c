...
/*                                                                                         */
static struct fmr_power_down *fmr_pd;

struct fmr_power_down {
    struct workqueue_struct *workqueue;
    struct delayed_work fmr_pd_delay;
};

static struct fm *wq_fm = NULL;
/*                                                                                         */
...
/*                                                                                         */
static void fmr_off_work_func(struct work_struct *work)
{
	fm_powerdown(wq_fm, 0);
}
/*                                                                                         */
...
/*                                                                                         */
		fmr_pd = kzalloc(sizeof(struct fmr_power_down), GFP_KERNEL);
		if (fmr_pd == NULL) {
				printk("FM_IOCTL_POWERDOWN : Can not allocate memory\n");
		}
		fmr_pd->workqueue = create_workqueue("fmr_pd_workqueue");	
		if (!fmr_pd->workqueue) {
				printk("FM_IOCTL_POWERDOWN : Unable to create workqueue\n");
		}
		INIT_DELAYED_WORK(&fmr_pd->fmr_pd_delay, fmr_off_work_func);
/*                                                                                         */
...
/*                                                                                         */
		wq_fm = fm;
		queue_delayed_work(fmr_pd->workqueue,&fmr_pd->fmr_pd_delay, msecs_to_jiffies(70));
