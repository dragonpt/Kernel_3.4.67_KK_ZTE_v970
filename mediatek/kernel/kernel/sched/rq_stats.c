...
static unsigned int heavy_task_threshold = 900; // max=1023
...
unsigned int sched_get_percpu_load(int cpu, bool reset)
{
	struct cpu_load_data *pcpu;
	unsigned int load = 0;
    unsigned long flags;
...
unsigned int sched_get_nr_heavy_task_by_threshold(unsigned int threshold)
{
	...
	for_each_online_cpu(cpu) {
		...
		list_for_each_entry(p, &cpu_rq(cpu)->cfs_tasks, se.group_node) {
			if (p->se.avg.load_avg_ratio >= threshold)
				count++;
		}
		...
		struct kobj_attribute *attr, char *buf)
{
	...
	for_each_possible_cpu(cpu) {
		load = sched_get_percpu_load(cpu, 1);
		len += snprintf(buf+len, max_len-len, "cpu(%d)=%d\n", cpu, load);
#if 0
		struct cpu_load_data *pcpu = &per_cpu(cpuload, cpu);
		len += snprintf(buf+len, max_len-len, "idle=%u, io=%u, wall=%u, %s\n",
	...
	len += snprintf(buf+len, max_len-len, "htask#=%u\n",
		sched_get_nr_heavy_task());
...
static int __init rq_stats_init(void)
{
	...
	for_each_possible_cpu(i) {
		...
		//if (cpu_online(i))
		//	pcpu->cur_freq = cpufreq_get(i); //1000
