...
#define BATCH_ERR(fmt, args...)	printk(BATCH_TAG"%s %d : "fmt, __func__, __LINE__, ##args)
...
#define EVENT_TYPE_SENSORTYPE							ABS_RZ
#define EVENT_TYPE_BATCH_VALUE          	ABS_RX
#define EVENT_TYPE_END_FLAG         			ABS_RY
...
{
	int (*get_data)(int handle, struct hwm_sensor_data *data);//sensor data is got one by one, return value: 1 stands for data read not finish 0 stands for read data done 
	int delay;//report latency for every sensor
	int flags;//reserved
...
extern void report_batch_finish(struct input_dev *dev, int sensor);
...
extern int batch_register_support_info(int handle, int support);
#endif
